...
ListEntry* partition(ListEntry* head, ListEntry* end, EntryCompareFunction cmpFunc) {
...
static void quickSort(ListEntry* head, ListEntry* end, EntryCompareFunction cmpFunc) {
	/*if (end != NULL && head != end && head != end->next) {
		ListEntry* pivot = partition(head, end, cmpFunc);
		quickSort(head, pivot->prev, cmpFunc);
		quickSort(pivot->next, end, cmpFunc);
	}*/
}
