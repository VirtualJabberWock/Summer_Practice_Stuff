...
WNDCLASSEX WndClass;    // структура класса
    ...
    WndClass.hIcon          = NULL; //LoadIcon (hIns, "MainIcon");  // иконка приложения
    WndClass.hCursor        = LoadCursor (NULL, IDC_IBEAM);
    WndClass.hbrBackground  = (HBRUSH) GetStockObject (BLACK_BRUSH);    // кисть для фона
    WndClass.lpszMenuName   = NULL;
    WndClass.lpszClassName  = AppWindowName;        // имя класса
    WndClass.hIconSm        = LoadIcon (hIns, TEXT("MainIcon"));
...
HWND hwnd;  // описатель окна
    ...
    if(hwnd == NULL){
        int a = MessageBox(NULL,"Ошибка создания основного окна приложения","",MB_OK);
        return NULL;
        }
...
PAINTSTRUCT ps;
    ...
    switch (iMsg){
        ...
        case WM_PAINT:
            ...
            pen = CreatePen(
                            PS_DASHDOTDOT,       // задаём тип линии (сплошная)
                            1,              // толщина линии
            ...
            LineTo(
                   hdc,             // описатель контекста устройства
                   2000,   // X-координата конечной точки
