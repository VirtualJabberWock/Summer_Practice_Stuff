...
LRESULT CALLBACK MyEditSimpleWndProc (HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
...
//
WNDPROC CreateNewClass (LPCSTR oldClassName, LPCSTR newClassName, WNDPROC newWndProc, HINSTANCE ins) {

WNDCLASS    wndclass;
WNDPROC     wndproc;

    GetClassInfo(NULL,oldClassName,&wndclass);
    wndclass.lpszClassName = newClassName;
    wndproc = wndclass.lpfnWndProc;
    wndclass.lpfnWndProc = newWndProc;
    wndclass.hInstance = ins;
    wndclass.hCursor = LoadCursor (NULL, IDC_CROSS); // курсор окна
...
HWND hChild;
    ...
    //glWndProc = CreateNewClass ("edit", "myedit", MyEditSimpleWndProc, hIns);
    ...
    switch (iMsg){
        ...
        case WM_CHAR:
            ...
            if ( (char)wParam == '\r') {
                ...
                return glWndProc (hwnd, iMsg, '#', lParam);
                }
        }
    //*/
